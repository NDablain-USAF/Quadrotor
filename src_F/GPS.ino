...
void loop() {
  ...
  if (FIX==true){
    ...
    if (Integrity>0){
      ...
      /*
      for (int i=altitudeSize-1;i>=0;i--){
        Serial.print((char)altitudeArray[i]);
      }
      Serial.print(" , ");
      */
      //Serial.println(latitude*1e8);
    }
  }
  else {
...
void displacements(double latitude, double longitude, double altitude){
  ...
  double c[2] = {0.995,0.005}; // Filter constants
  double precision = 1e-9; // Expand 
  double precisionAlt = 1e-7;
  ...
  double Altitude = (altitude*(1/precisionAlt))*c[1] + AltitudeLast*c[0];
  if (biasControl==300000){
    ++biasControl;
  }
  else if (biasControl<300000){
    ++biasControl;
    if ((biasControl==1000)||(biasControl==150000)){
